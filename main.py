...
def calculator():
    if request.method == 'POST':
        try:
            ...
            num_variables = max(1, min(100, num_variables))
            num_constraints = max(1, min(50, num_constraints))

            optimization_type = request.form['optimization_type']

            objective_coefficients = [float(request.form.get(f'coef_{i}')) for i in range(1, num_variables + 1)]

            constraint_entries_list = [[float(request.form.get(f'constraint_{i}_{j}'))
                                        for j in range(1, num_variables + 1)]for i in range(1, num_constraints + 1)]
            ...
            problem = pulp.LpProblem("Problema de Programación Lineal",pulp.LpMaximize if optimization_type == 'Maximizar' else pulp.LpMinimize)
            ...
            return "Invalid input. Please check your data.", 400
    ...
    return render_template('calculator.html', num_constraints=num_constraints, num_variables=num_variables, optimization_type=optimization_type)
